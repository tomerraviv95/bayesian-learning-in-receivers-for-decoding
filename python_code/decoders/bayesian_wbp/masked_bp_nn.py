...
class MaskedOutputLayer(torch.nn.Module):
    ...
    def forward(self, x, mask_only=False, phase=Phase.TEST):
        ...
        x_after_dropout = dropout_ori(x, self.dropout_logits, u)
        if mask_only:
            return torch.matmul(x_after_dropout, self.w_output_mask)
        else:
            return torch.matmul(x_after_dropout, self.w_output_mask * self.output_weights)
